...
- `docs/pitfalls-and-lessons.md` is a running log of debugging pitfalls and the generalizable lesson each one left behind (tautological eval metrics, LangGraph silently dropping undeclared `AgentState` fields, regex-chasing an LLM's output-wrapping quirks instead of parsing for the payload, shared reasoning/answer token budgets, under-provisioned local infra memory caps, credential-format assumptions, and more). Read it when investigating a bug that doesn't have an obvious cause. Add a short entry (what happened / root cause / lesson) whenever a fix here turns out to need a second pass, or a bug's root cause wasn't obvious from its symptom — see the doc's own header for the entry format.
